...
async function scrapeInstagramData(username) {
  try {
    ...
    // Check if the login was successful by looking for an element that only appears when logged in
    try {
      await page.waitForSelector('header', { timeout: 5000 }); // Waiting for header as an indication of successful login
      console.log('Login successful, navigating to user profile...');
    } catch (loginError) {
      console.error('Login failed. Check credentials or Instagram block:', loginError);
      await browser.close();
    ...
    try {
      await page.waitForSelector('header section ul li span', { timeout: 5000 });
      console.log('User profile found, scraping data...');
    } catch (scrapeError) {
      console.error('Failed to scrape user profile data:', scrapeError);
      await browser.close();
      return null; // Exit if scraping fails
    }
...
app.get('/profile/:username', async (req, res) => {
  const { username } = req.params;
  console.log(`Received request for profile: ${username}`);
